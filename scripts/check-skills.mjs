...
async function validateSkill(entry) {
...
try {
  ...
  if (entries.length === 0) fail("repository", "no skill directories found");
  for (const entry of entries) await validateSkill(entry);

  if (failures.length) {
    failures.forEach(({ skill, message }) => console.error(`FAIL ${skill}: ${message}`));
    process.exitCode = 1;
  } else {
    console.log(`PASS ${entries.length} skill(s) validated.`);
  }
} catch (error) {
